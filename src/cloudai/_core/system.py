...
class System(ABC):
    ...
    @abstractmethod
    def get_stdout(self, job: "BaseJob") -> Optional[str]:
        """
        Retrieve the stdout content for a given job.

        Args:
            job (BaseJob): The job for which to retrieve stdout.

        Returns:
            Optional[str]: The stdout content as a string, or None if it cannot be retrieved.

        Raises:
            NotImplementedError: Raised if the method is not implemented in a subclass.
        """
        error_message = (
            "Retrieving stdout method is not implemented. All subclasses of the System class must implement the "
            "'get_stdout' method to provide stdout retrieval functionality."
        )
        logging.error(error_message)
        raise NotImplementedError(error_message)

    @abstractmethod
    def get_stderr(self, job: "BaseJob") -> Optional[str]:
        """
        Retrieve the stderr content for a given job.

        Args:
            job (BaseJob): The job for which to retrieve stderr.

        Returns:
            Optional[str]: The stderr content as a string, or None if it cannot be retrieved.

        Raises:
            NotImplementedError: Raised if the method is not implemented in a subclass.
        """
        error_message = (
            "Retrieving stderr method is not implemented. All subclasses of the System class must implement the "
            "'get_stderr' method to provide stderr retrieval functionality."
        )
        logging.error(error_message)
        raise NotImplementedError(error_message)
