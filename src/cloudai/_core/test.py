...
class Test:
    ...
    def prepare_job(self, output_path: Optional[str] = None, job_name: Optional[str] = None, **kwargs) -> Any:
        """
        Prepare the job specification using the associated test template.

        Args:
            output_path (Optional[str]): Path to the output directory (optional).
            job_name (Optional[str]): The name of the job (optional).
            **kwargs: Additional arguments that may be required by the test template's prepare_job method.
        ...
        return self.test_template.prepare_job(
