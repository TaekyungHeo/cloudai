...
class KubernetesRunner(BaseRunner):
    ...
    def _submit_test(self, test: Test) -> KubernetesJob:
        ...
        return KubernetesJob(self.mode, self.system, test, job_namespace, job_name, job_output_path)
    ...
    def kill_job(self, job: BaseJob) -> None:
        ...
        k8s_system: KubernetesSystem = cast(KubernetesSystem, self.system)
        pod_name = k_job.get_id()
        assert isinstance(pod_name, str)
        k8s_system.store_pod_logs(k_job.namespace, pod_name, k_job.output_path)
