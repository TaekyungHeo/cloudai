...
class StandaloneSystem(System):
    ...
    def get_stdout(self, job: BaseJob) -> Optional[str]:
        """
        Retrieve the stdout content for a given standalone job.
        ...
        """
        try:
            stdout_path = f"{self.output_path}/{job.get_id()}/stdout.txt"
            with open(stdout_path, "r") as stdout_file:
                return stdout_file.read()
        except FileNotFoundError:
            logging.error(f"stdout.txt file not found for job {job.get_id()}.")
            return None

    def get_stderr(self, job: BaseJob) -> Optional[str]:
        """
        Retrieve the stderr content for a given standalone job.
        ...
        """
        try:
            stderr_path = f"{self.output_path}/{job.get_id()}/stderr.txt"
            with open(stderr_path, "r") as stderr_file:
                return stderr_file.read()
        except FileNotFoundError:
            logging.error(f"stderr.txt file not found for job {job.get_id()}.")
            return None
